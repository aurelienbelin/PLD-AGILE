...
public class PlanVille {
     ...
     * @return Une HashMap réalisation le hashage suivant : 
     * Intersection i1 => (i2,v)
     * où i2 est l'Intersection précédente et v un float représent le cout pour 
     * aller de p à i1.
     */
    protected Map<Intersection, Pair<Intersection, Float>> dijkstra(PointPassage p){
        ...
        //Structure de données contenant à la fois la précédence et le cout 
        //depuis le départ
        Map<Intersection, Pair<Intersection,Float>> tab = new HashMap<>();
        //File de priorité
        PriorityQueue<Intersection> nonVus = new PriorityQueue<>(
        ...
        while(!nonVus.isEmpty()){
            ...
            while(troncIt.hasNext()){
                ...
                if(tab.containsKey(arc.getFin())){
                    ...
                    if(tab.get(arc.getFin()).getValue()>(
                            arc.getLongueur()+tab.get(origine).getValue())){
                        Pair<Intersection,Float> predecesseur = new Pair(
                                origine,arc.getLongueur()+
                                        tab.get(origine).getValue());
     ...
     * @param listePoints - L'ensemble des points de passage (livraison & 
     * entrepot) par où on doit passer.
     * @return Une List de Chemin. Il est garanti que la liste sera de taille 
     * n² avec n le nombre de point de passage. Dans cette liste, il existe 
     * toujours un unique chemin menant de n_i à n_j, ce chemin étant le plus 
     * court possible dans la ville.
     */
    protected List<Chemin> dijkstraTousPoints(List<PointPassage> listePoints){
        List<Chemin> graph = new ArrayList<>();
        if(listePoints==null || listePoints.size()<2){
            return graph;
        }
        for(PointPassage depart : listePoints){
            if (depart!=null){
                Map<Intersection, Pair<Intersection,Float>> tab = 
                        dijkstra(depart);
                    ...
                    for(PointPassage arrivee : listePoints){ 
                        if(arrivee!=depart && arrivee!=null){
                            ...
                            if(i.equals(arrivee.getPosition())){
                                ...
                                Chemin chemin = reconstruireChemin(depart, 
                                        arrivee, tab);
                                //ajouter a la liste des chemins entre pts de 
                                //passages
                                if (chemin!=null){
                                    graph.add(chemin); 
                                }
    ...
    /**
     * Reconstruit le chemin menant d'un point de départ à un point d'arrivée à
     * l'aide de la structure de précédence générée
     * par dijkstra.
     * @param depart - Le Point de passage duquel le chemin doit partir.
     * @param arrivee - Le point de passage où l'on souhaite arriver.
     * @param tab - Une Map liant à une intersection son cout et sa précédence. 
     * Il s'agit de la structure obtenue après algorithme de Dijkstra.
     * @return Le Chemin menant du départ à l'arrivée.
     */
    protected Chemin reconstruireChemin(PointPassage depart, 
            PointPassage arrivee, 
            Map<Intersection, 
                    Pair<Intersection, Float>> tab){
        List<Troncon> trChemin = new ArrayList<>();
        //retrouver point précédent
        Intersection ptInter1;
        try{
            ptInter1 = tab.get(arrivee.getPosition()).getKey();
        } catch(Exception e){
            return null; //L'arrivee ne fait pas partie des precedences, 
    ...
    /**
     * Construit un nouveau plan de ville à partir d'un ensemble d'intersections 
     * reliées par un ensemble de troncons.
